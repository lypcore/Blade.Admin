...
export default {
  ...
  methods: {
    init() {
      ...
      this.$http.post('/BaseManage/BaseAction/GetActionTreeList', {}).then(resJson => {
        if (resJson.Success) {
          this.actionsTreeData = resJson.Data
        }
      })
      this.$http.post('/BaseManage/BaseAction/GetAllActionList', {}).then(resJson => {
        if (resJson.Success) {
    ...
    openForm(id) {
      ...
      if (id) {
        this.$http.post('/BaseManage/BaseRole/GetTheData', { id: id }).then(resJson => {
    ...
    handleSubmit() {
      this.$refs['form'].validate(valid => {
        ...
        this.$http.post('/BaseManage/BaseRole/SaveData', this.entity).then(resJson => {
