<template>
  <a-card :bordered="false">
    <div class="table-operator">
      <a-button v-if="hasPerm('BaseUser.Add')" type="primary" icon="plus" @click="hanldleAdd()">新建</a-button>
      <a-button
        v-if="hasPerm('BaseUser.Delete')"
        type="primary"
        icon="minus"
    ...
    >
      <span slot="action" slot-scope="text, record">
        <template>
          <template v-if="hasPerm('BaseUser.Edit')">
            <a @click="handleEdit(record.Id)">编辑</a>
            <a-divider type="vertical" />
          </template>
          <a v-if="hasPerm('BaseUser.Delete')" @click="handleDelete([record.Id])">删除</a>
        </template>
      </span>
    </a-table>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.loading = true
      this.$http
        .post('/BaseManage/BaseUser/GetDataList', {
    ...
    submitDelete(ids, resolve, reject) {
      this.$http.post('/BaseManage/BaseUser/DeleteData', ids).then(resJson => {
        resolve()
