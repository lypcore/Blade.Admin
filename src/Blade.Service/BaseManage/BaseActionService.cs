...
namespace Blade.Service.BaseManage
{
    public class BaseActionService : BaseService<BaseAction>, IBaseActionService, ITransientDependency
    {
        ...
        public BaseActionService(IDbAccessor db, IMapper mapper)
            : base(db)
        {
            _mapper = mapper;
        }
        ...
        public async Task<List<BaseAction>> GetDataListAsync(BaseActionsInputDTO input)
        {
            var q = GetIQueryable();
            q = q
        ...
        public async Task<List<BaseActionDTO>> GetTreeDataListAsync(BaseActionsInputDTO input)
        {
            var qList = await GetDataListAsync(input);

            var treeList = qList.Select(x => new BaseActionDTO
            {
                Id = x.Id,
            ...
            async Task SetProperty(List<BaseActionDTO> _list)
            {
        ...
        public async Task<BaseAction> GetTheDataAsync(string id)
        {
        ...
        {
            await InsertAsync(_mapper.Map<BaseAction>(input));
        ...
        {
            await UpdateAsync(_mapper.Map<BaseAction>(input));
            await SavePermissionAsync(input.Id, input.permissionList);
        }
        ...
        public async Task SavePermissionAsync(string parentId, List<BaseAction> permissionList)
        {
