...
namespace Blade.Service.Service
{
    public class BaseRoleService : BaseService<BaseRole>, IBaseRoleService, ITransientDependency
    {
        ...
        public BaseRoleService(IDbAccessor db, IMapper mapper)
            : base(db)
        {
            _mapper = mapper;
        }
        ...
        public async Task<PageResult<BaseRoleInfoDTO>> GetDataListAsync(PageInput<RolesInputDTO> input)
        {
            var where = LinqHelper.True<BaseRole>();
            ...
            var page = await GetIQueryable()
                .Where(where)
                .ProjectTo<BaseRoleInfoDTO>(_mapper.ConfigurationProvider)
            ...
            async Task SetProperty(List<BaseRoleInfoDTO> _list)
            {
                var allActionIds = await Db.GetIQueryable<BaseAction>().Select(x => x.Id).ToListAsync();

                var ids = _list.Select(x => x.Id).ToList();
                var roleActions = await Db.GetIQueryable<BaseRoleAction>()
        ...
        public async Task<BaseRoleInfoDTO> GetTheDataAsync(string id)
        {
        ...
        public async Task AddDataAsync(BaseRoleInfoDTO input)
        {
            await InsertAsync(_mapper.Map<BaseRole>(input));
        ...
        [DataRepeatValidate(new string[] { "RoleName" }, new string[] { "角色名" })]
        [Transactional]
        public async Task UpdateDataAsync(BaseRoleInfoDTO input)
        {
            await UpdateAsync(_mapper.Map<BaseRole>(input));
            await SetRoleActionAsync(input.Id, input.Actions);
        }
        ...
        public async Task DeleteDataAsync(List<string> ids)
        {
            ...
            await Db.DeleteAsync<BaseRoleAction>(x => ids.Contains(x.RoleId));
        }
        ...
        private async Task SetRoleActionAsync(string roleId, List<string> actions)
        {
            ...
            await Db.DeleteAsync<BaseRoleAction>(x => x.RoleId == roleId);
