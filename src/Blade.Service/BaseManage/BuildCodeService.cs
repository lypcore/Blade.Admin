...
namespace Blade.Service.BaseManage
{
    ...
    {
        public BuildCodeService(IDbAccessor db, IHostingEnvironment evn)
            : base(db)
        {
...
$@"        <a-form-model-item label=""{aField.Description}"" prop=""{aField.Name}"">
        ...
        {
            string nameSpace = $@"Blade.Entity.{_areaName}";
            string filePath = Path.Combine(_solutionPath, "Blade.Entity", _areaName, $"{tableName}.cs");
        ...
        private BaseDbLink GetTheLink(string linkId)
        {
            BaseDbLink resObj = new BaseDbLink();
        ...
        {
            string tmpFileText = File.ReadAllText(Path.Combine(_solutionPath, "Blade.Api", "BuildCodeTemplate", templateFileName));
